...
def rollout(grid, num_simulations=5): # simulation
    total_score = 0
    # run multipule simulations and averge them
    for _ in range(num_simulations):
        simulation_grid = deepcopy(grid)
        while not simulation_grid.is_gameover():
            move = random.choice(["left", "right", "up", "down"])
            getattr(simulation_grid, f"shift_{move}")()
...
def monte_carlo_tree_search(initial_grid, iterations=20, num_rollouts_per_node=5):
    ...
    for _ in range(iterations):
        ...
        score = rollout(deepcopy(node.grid), num_rollouts_per_node)
